...
- Latest implementation (2026-05-16, session asset-dialog-redesign): **AssetDialog category-first redesign + edit mode field filtering**. New 2-step flow for adding assets: Step 1 = visual type picker (7 cards with Lucide icons + Italian descriptions); Step 2 = smart form showing only relevant fields for the chosen type. Asset class auto-derived via `TYPE_TO_CLASS` map — user never picks both type and class. `DialogTitle` is dynamic and grammatically gendered (Nuova Azione, Nuovo ETF, etc.). Same field-visibility helpers (`newAsset_*`) now apply in edit mode too, replacing the previous "show everything in edit" approach. Also fixed stale form data bug: `useEffect([asset, reset, open])` + `if (!open) return` guard + complete field list in the `else` branch (was missing all bond fields + isin). Key file: `components/assets/AssetDialog.tsx`.
- Previous (2026-05-15, session radix-umbrella-import): **Radix UI import unification** in `components/ui/`. Migrated 10 files from `@radix-ui/react-*` namespace imports to `radix-ui` umbrella named imports. Removed 12 direct `@radix-ui/*` entries from `package.json`. Convention: `{ X as XPrimitive } from "radix-ui"` using PascalCase.
...
**Last updated**: 2026-05-16 (session asset-dialog-redesign — category-first AssetDialog redesign + edit mode field filtering)
