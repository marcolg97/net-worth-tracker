...
- Latest implementation (2026-05-17, session refactor-usewatch): **React Compiler compatibility — `watch()` → `useWatch()` migration** across all 4 form dialogs. `AssetDialog` (34 calls), `ExpenseDialog` (11 calls, `"use no memo"` directive removed), `CategoryManagementDialog` (5 calls), `DividendDialog` (4 calls). Rule: `useWatch({ control, name })` for render-time reads; `getValues()` for event handlers. JSX IIFEs are render-time — use `useWatch` top-level variables inside them. Also added `DialogDescription` to `DividendDialog` (Radix a11y warning fix).
- Previous (2026-05-16, session expense-dialog-redesign): **ExpenseDialog 2-step progressive disclosure redesign**. Step 1 = visual type picker (4 cards: Spesa Variabile/ShoppingCart, Spesa Fissa/CalendarClock, Debito·Rata/CreditCard, Entrata/ArrowDownToLine); Step 2 = smart form with only relevant fields. Edit mode lands directly on Step 2 with type as locked `Badge variant="outline"`. `AnimatePresence mode="wait"` for step transitions. `InstallmentPreview` microcopy in Italian ("N rate da €X"). Key file: `components/expenses/ExpenseDialog.tsx`.
- Previous (2026-05-16, session asset-dialog-redesign): **AssetDialog category-first redesign + edit mode field filtering**. 2-step flow: Step 1 = visual type picker (7 cards); Step 2 = form with fields filtered by type. `TYPE_TO_CLASS` auto-derives `AssetClass`. Edit mode uses same field-visibility helpers (`newAsset_*`). Fixed stale form data bug: `useEffect([asset, reset, open])` + `if (!open) return` guard. Key file: `components/assets/AssetDialog.tsx`.
- Previous (2026-05-13, session portfolio-exposure-breakdown): **Esposizione Portfolio v1**. Collapsible section at bottom of Allocazione with 3 tabs (top companies, sectors, ETF issuers). Click-to-expand calculation drill-down ("X% di €Y = €Z" per source ETF). `GET /api/portfolio/exposure`, 24h Firestore cache, `?force=true` bypass. New files: `types/exposure.ts`, `lib/server/portfolioExposureService.ts`, `app/api/portfolio/exposure/route.ts`, `lib/hooks/usePortfolioExposure.ts`, `components/allocation/ExposureSection.tsx`.
- Previous (2026-05-08, session jiggly-deer): **Mesi+/- denominator display** in benchmark table ("X/Y" format, `totalMonths` field). **ECB rate auto-sync** via FRED ECBDFR — Sharpe/Sortino use period-average ECB rate, cached in `ecb-rate-cache/deposit-rate` (7d TTL). **Benchmark comparison** ("Confronto con Portafogli Modello"): 11-column risk/return table, 6 model portfolios, indexed growth chart, optional EUR conversion. Env var: `FRED_API_KEY`.
...
- Allocazione now presents a more readable drill-down path on desktop and a steadier mobile sheet experience, with each drill-down level reopening from the top and progress bars using centered target markers. Bottom section "Esposizione Portfolio" (2026-05-13): cross-ETF breakdown by company / sector / ETF issuer with click-to-expand calculation drill-down ("X% di €Y = €Z" per source + Totale row). Lazy-loaded server-side via `GET /api/portfolio/exposure` with 24h Firestore cache and `?force=true` bypass for the "Aggiorna" button.
...
**Last updated**: 2026-05-17 (session refactor-usewatch — watch() → useWatch() migration for React Compiler compatibility)
